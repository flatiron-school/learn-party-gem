...
describe LearnTogether do
  ...
  context "groups of n students" do
    ...
    context "when given a collection of student objects" do
      ...
      describe "with a batch of 20 and groups of 2" do
        describe "random" do
          students = collection_of_student_objects.slice(0,20)
          groups = LearnTogether::GroupMaker.new.run(collection: students, groups_of: 2, sort_type: "random")

          it 'give a batch of 20 students groups students in to the correct number of groups of two' do
            expect(groups.first.size).to eq 2
            expect(groups.length).to eq 10
            expect(groups.last.length).to eq 2
          end

          it "should give the right distribution of students" do
            expect(calculate_random_groups(groups)).to match_array(groups_of_two_random_from_20)
          end
        end
      end

      describe "with a batch of 21 and groups of 2" do
        describe "random" do
          students = collection_of_student_objects.slice(0,21)
          groups = LearnTogether::GroupMaker.new.run(collection: students, groups_of: 2, sort_type: "random")

          it 'gives a batch of 21 students groups students in to the correct number of groups of two' do

            expect(groups.first.size).to eq 2
            expect(groups.length).to eq 10
            expect(groups.last.length).to eq 3

            expect(calculate_random_groups(groups)).to match_array(groups_of_two_random_from_21)
          end
        end
      end

      describe "with a batch of 20 and groups of 3" do
        describe "random" do
          students = collection_of_student_objects.slice(0,20)
          groups = LearnTogether::GroupMaker.new.run(collection: students, groups_of: 3, sort_type: "random")


          it "gives a batch of 20 students groups students in to the correct number of groups of three" do
      ...
      describe "with a batch of 35 and groups of 2" do
        describe "random" do
